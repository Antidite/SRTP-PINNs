...
u_bound = exact_calculation(x_train, t_initial, nu)
...
for epoch in range(1001):
    ...
    du_dx = du[:, 1].unsqueeze(1)
    d2u_dx = torch.autograd.grad(du_dx, input_train, torch.ones_like(du_dx), create_graph=True)[0]
    d2u_dx2 = d2u_dx[:, 1].unsqueeze(1)

    u = u.unsqueeze(1)
    loss_physics = torch.mean((du_dt + u * du_dx - nu * d2u_dx2) ** 2, dim=1)
...
pinn.eval()
with torch.no_grad():
    u_predict = torch.fft.irfft2(pinn(input_train), s=(len(x_train), len(t_train))).real.reshape(len(t_train), len(x_train))
